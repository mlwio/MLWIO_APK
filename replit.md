...
## Overview
MLWIO is a multi-platform Flutter application designed to provide a YouTube-like video streaming experience. It supports movies and series with episode playback, auto-play functionality, and integrates Google Drive for video embedding. The project aims to deliver a high-quality user interface with smooth animations, Material 3 design, and a focus on scalability and user experience, making it production-ready for UI/UX demonstration and future API integration.
...
- **Core Screens:**
    ...
    - **Video Player Screen:** Embeds Google Drive videos with playlist controls and auto-ads.
...
- **Content Models:** `ContentItem`, `MovieContent`, `SeriesContent`, `Season`, and `Episode` for structured data.
- **Services:** `ApiService` for real API integration, `AuthService` for Google authentication, `AdMobService` for simplified ad display.
...
- **Core Functionality:**
    ...
    - Google Drive Video Playback via InAppWebView.
...
- **`url_launcher`**: For launching external URLs, potentially for Google Drive video links.
...
- **MLWIO API**: Custom backend API hosted at `https://mlwioapi-production.up.railway.app` for content retrieval via the `/api/content` endpoint.
